...
from utils.model_util import load_vgg19
...
def _predict(result, pred_filenames):
    ...
    pd.DataFrame({"filename": pred_filenames}).to_csv("../result/vgg19/test_filename.csv", index=None)
    np.save("../result/vgg19/real_test_2_result.npy", result)

    write_result2df_json(result, df_test=pd.DataFrame({"filename": pred_filenames}),
                         df_filename="../result/vgg19/real_test_2_result.csv",
                         json_filename="../result/vgg19/real_test_2_result.json")

model = load_vgg19(input_size=input_size)

adam_optimizer = Adam(lr=1e-4)
sgd_optimizer = SGD(lr=5*1e-4, momentum=0.8, decay=1e-6)
...
for fold in range(kfold):
    ...
    train_model_imagedatagen(
            ...
            train_gen=train_gen[0].flow_from_directory(train_dir,
                        shuffle=True,target_size=(input_size, input_size), batch_size=batch_size),
            valid_gen = train_gen[1].flow_from_directory(valid_dir,
                        shuffle=True,target_size=(input_size,input_size), batch_size=batch_size),
            len_train=len_train, len_valid=len_valid, batch_size=batch_size, epochs=50)
