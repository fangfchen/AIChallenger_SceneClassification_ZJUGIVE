...
from utils.model_util import load_resnet50, train_model_imagedatagen
...
model_save_file = "weight/resnet50"
result_save_dir = "../result/resnet50"
...
def _predict(result, pred_filenames):
    ...
    pd.DataFrame({"filename": pred_filenames}).to_csv("../result/resnet50/test_filename.csv", index=None)
    np.save("../result/resnet50/real_test_2_result.npy", result)

    write_result2df_json(result, df_test=pd.DataFrame({"filename": pred_filenames}),
                         df_filename="../result/resnet50/real_test_2_result.csv",
                         json_filename="../result/resnet50/real_test_2_result.json")

model = load_resnet50(input_size=input_size)
...
for fold in range(kfold):
    ...
    checkpoint_file = os.path.join("weights", "resnet50", "fold_"+str(fold))+".hdf5"
    ...
    train_model_imagedatagen(model=model, batch_size = batch_size, checkpoint_file=checkpoint_file,
                         ...
                         valid_gen=train_gen[1].flow_from_directory(valid_dir, shuffle=True,
                                target_size=(input_size, input_size), batch_size=batch_size),
                         len_train=len(df_train_all), len_valid=len(df_valid), epochs=epochs)
